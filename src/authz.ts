...
export function isNamespaceAllowed(namespace: string | undefined): boolean {
  if (!namespace) return true; // allow cluster-scoped if namespace not applicable
  const allow = parseCsv(process.env.MCP_K8S_NAMESPACE_ALLOWLIST);
  if (allow.length === 0) return true; // no allowlist set => allow
  return allow.includes(namespace);
}

export function isKindAllowed(kind: string | undefined): boolean {
  if (!kind) return true;
...
export class McpGuardError extends Error {
  ...
  constructor(message: string, code = 'GUARD_VIOLATION', details?: Record<string, unknown>) {
    super(message);
    this.code = code;
...
export function enforceMutatingGuards(
  logger: pino.Logger,
  toolName: string,
  options: { namespace?: string; kind?: string; dryRun?: boolean }
) {
  if (isReadOnly()) {
...
export function rateLimit(toolName: string, burst = 10, rate = 5) {
  const key = toolName;
