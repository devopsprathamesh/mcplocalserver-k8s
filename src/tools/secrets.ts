...
export function registerSecretTools(server: McpServer, logger: pino.Logger) {
  const clients = getClients(logger);

  server.tool(
    'secrets.get',
    'Get a secret (redacted by default)',
    GetSecretParams.shape,
    async (args) => {
      ...
      for (const k of keys) {
        const v = body.data?.[k];
        if (showValues && typeof v === 'string') data[k] = v; // base64 values as-is
        else data[k] = redactSecret(v);
      }
  ...
  server.tool(
    'secrets.set',
    'Create/update a secret with provided keys (values never logged)',
    SetSecretParams.shape,
    async (args) => {
