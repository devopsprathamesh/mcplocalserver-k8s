...
function summarizePod(pod: k8s.V1Pod) {
...
export function registerWorkloadTools(server: McpServer, logger: pino.Logger) {
  const clients = getClients(logger);

  server.tool(
    'pods.listPods',
    'List pods with optional selectors',
    ListPodsParams.shape,
    async (args) => {
  ...
  server.tool(
    'pods.get',
    'Get a pod summary including containers and events',
    GetPodParams.shape,
    async (args) => {
      rateLimit('pods.get');
      ...
      let events: Array<{ type?: string | null; reason?: string | null; message?: string | null; age?: string | Date | null }> = [];
      try {
      ...
      const code = await new Promise<number>((resolve, reject) => {
        clients.exec
          .exec(namespace, name, container || '', command, null, null, null, false, (status) => {
            const c = (status as any)?.status === 'Success' ? 0 : 1;
            resolve(c);
          })
          .catch((e) => reject(e));
      });
