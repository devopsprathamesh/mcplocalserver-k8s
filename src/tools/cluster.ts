...
export function registerClusterTools(server: McpServer, logger: pino.Logger) {
  const clients = getClients(logger);

  server.tool(
    'cluster.health',
    'Get basic cluster health and version',
    async () => {
      ...
      return {
        content: [{ type: 'text', text: JSON.stringify({ status: 'ok', clusterVersion: (info as any).gitVersion, serverAddress: clients.kubeConfig.getCurrentCluster()?.server, timestamp: new Date().toISOString() }) }],
      };
    }
  );
  ...
  server.tool(
    ...
    'List kubeconfig contexts and current selection',
    async () => {
